...
export default function SystemDiagramCanvas() {
  ...
  return (
    <div className="relative h-[600px] scale-90 top-[-60px]">
      ...
      <div className="absolute left-[0px] top-[256px] flex items-center gap-4">
        <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-amber-500 to-orange-500 flex items-center justify-center shadow-lg">
          <svg width="32" height="32" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <line x1="17" y1="10" x2="17" y2="14" stroke="white" strokeWidth="2" strokeLinecap="round"/>
          </svg>
        </div>
        <div>
      ...
      {/* Connection Lines */}
      <svg key={animKey} className="absolute inset-0 w-full h-full pointer-events-none" style={{ zIndex: 1 }}>
        <defs>
          {/* 메인 경로 정의 */}
          {/* 방전 모드: ESS에서 계통으로 (위쪽) */}
          <path id="mainPathDischarge" d="M 170 450 L 170 170" />
          {/* 방전 모드: ESS에서 부하로 (분기) */}
          <path id="branchPathDischarge" d="M 170 300 L 320 300 L 320 240 L 435 240" />
          ...
          <>
            ...
            {[0, 0.25, 0.5, 0.75].map((offset, index) => (
              <g key={`arrow-pcs-ess-${index}`}>
                <polygon points="0,-6 12,0 0,6" fill="#ef4444" transform="rotate(90)" visibility="hidden">
                  <animate
                    attributeName="visibility"
                    to="visible"
                    begin={`${offset * 5}s`}
            ...
            {/* PCS 포인트 표시 */}
            <circle cx="170" cy="295" r="5" fill="#ef4444" className="animate-pulse" />
          </>
        )}
      </svg>
    </div>
